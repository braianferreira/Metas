...
if (tempoFinal > 0){
 ...
 function atualizarCronometro(){
    //laço de repetição para interagir com todos objetivos
    for (let i=0;i<contadores.length; i++){
        contadores[i].textContent= calculaTempo(tempoObjetivo[i]); //textContent mostra na tela a meta
    }

    
 }
 function comecaCronometro90{
     atualizarCronometro(0);//chamada da função criada anteriormente dentro desta função
     sentInterval(atualizarCronometro,1000);//função que faz a contagem do tempo a cada seg.
 }
 comecaCronometro(); //chamada da função que inicia o cronõmetro